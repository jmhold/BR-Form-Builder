				    ...
				    $( "#sortable" ).sortable({
						...
						$.ajax({
							...
							success: function(data){	
								 if(data.check==0){  alertMessage('error','Error ;)) Try  Agian'); return false;}
								 if(data.check==1){  alertMessage('success','Sortable Images Success'); return false;}
							},
	  ...
	  function albumDelete(datavalue,name){
			  ...
			  'action': function(){
								...
								$.ajax({
									...
									data: datavalue,
									success: function(data){	
										  if (data.check == "0"){ alertMessage('error','Error Delete)) Try  Agian'); }	   
										  if (data.check == "1"){
											  $('#albumsLoad').fadeOut(500,function(){
									...
									success: function(data){	
										  ...
										  if (data.check == "1"){
											  ...
											  $('#albumsLoad').fadeOut(function(){		  
													  if(data.thumb){
														  $('#image-albumPreview').attr('src','<?php echo 'images/icon/empty_album.jpg'?>');
													  }	
				...
				$('#editAlbum.editOff').live('click',function(){													   
                        ...
                        <div class="albumImagePreview"  style="display:block">
                              ...
                              <div class="picPreview"><img id="image-albumPreview" title="Drop Image Here"  src="<?php echo $thumb['thumbnail']?>" alt="Image Preview"  /></div>
                              <div class="clear"></div>
                              <div class="hr"></div>
                              
    							  <input type="hidden" name="id_edit" id="id_edit"  value="<?php echo $fecth_album[id]?>" />
                                  <input type="hidden" name="thumbPreview" id="thumbPreview"  />
                                 <div class="tip">
                                <input type="text" name="name" id="name"  class="validate[required] "  title="Album name" style="width:146px" value="<?php echo $fecth_album[name]?>" maxlength="35" />
                                </div>
                                  <div class="hr"></div>
                                  <ul class="uibutton-group">
                                    <a class="uibutton  normal save_" >save</a>
                                    <a class="uibutton  albumDelete  special " rel="<?php echo $fecth_album[id]?>" name="<?php echo $fecth_album[name]?>" >Delete </a>
                                  </ul>
                                  <div class="hr"></div>
                                  </form>
                                        ...
                                        <li class="albumImage" id="posi_<?=$row[id]?>" >
                                            <div class="picHolder">
                                                <span class="image_highlight"></span>
                                                  <a href="<?php echo $config['imageDir'].'/m/'.$row['filename']?>" rel='glr'></a>	
                                                  <img src="<?php echo $config['imageDir'].'/s/'.$row['filename']?>"  title="Drag This Photo" />
                                            <div class="picTitle"><?=$row['title']?></div>
                                            <ul class="dataImg"><!--// This data images with your call to php or SQL -->
                                                    <li><?php echo $row[id]?></li><!--// This id images-->
                                                    <li><?php echo $row[title]?></li><!--// This name images-->
                                                    <li><?php echo $row[albumid]?></li><!--// This album id images-->
                                            </ul>   
                                            </div>
                                        </li>
									  <?php }?>
